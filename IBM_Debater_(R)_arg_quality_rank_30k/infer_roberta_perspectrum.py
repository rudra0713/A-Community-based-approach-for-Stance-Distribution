...
#               'national_security']
topic_list = ['Abolish_nuclear_weapons', 'All_nations_have_a_right_to_nuclear_weapons',
              'Animal_testing_for_medical_research_should_be_allowed.',
              'Performing_tests_on_animals_must_be_banned',
              'Social_networking_sites_are_good_for_our_society',
              'There_is_a_need_for_developing_tactical_nuclear_weapons']
...
def evaluate(model, eval_dataloader):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            ...
            predictions = outputs.logits.cpu().numpy()  # Remove squeeze() temporarily

            print(f"predictions: {predictions}")
            predictions = outputs.logits.cpu().numpy()

            # Ensure we always have a 1D array, regardless of input shape
            predictions = predictions.reshape(-1)  # This flattens any shape into 1D
            all_predictions.extend(predictions)
            # # Handle both single predictions and sequences
            # if predictions.ndim == 0:  # scalar value
            #     all_predictions.append(predictions.item())
            # elif predictions.ndim == 1:  # 1D array
            #     all_predictions.extend(predictions)
            # else:
...
with open(
        "/scratch/rrs99/Stance_Distribution/community_detection_main_classes/output_survey_" + cur_topic + "_perspectrum_discourse_arguments.json",
        "r") as file_j:
